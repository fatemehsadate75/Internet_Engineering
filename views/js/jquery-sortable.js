...
function saveSort(){
	  ...
	  $('.sorted_table tbody tr').each(function(index){
		  newindex = index+1;
		  sortArray += $(this).data('id')+':'+newindex+'-'; 
	  });
	var query = $.ajax({
	  type: 'POST',
	  url: ranginesmspresta_ajax_url,
	  data: 'method=savepreparedsmssort&sort=' + sortArray,
	  dataType: 'json',
	  beforeSend: function(){
		  $('.sorted_table_save').html('<i class="process-icon-loading"></i>');
	  },
	  success: function(json) {
		  if(json.result == 1) $('.sorted_table_save').html('ترتیب جدید ذخیره شد. برای تغییر ترتیب قرارگیری پیامک های پیش فرض در لیست های موجود در افزونه روی یک سطر جدول بالا کلیک کرده و بالا یا پایین بکشید.');
	  }
	});
};
// Sortable rows
$('.sorted_table').sortable({
  ...
  onDrop: function  ($item, container, _super) {
    ...
    _super($item, container);
	$('.sorted_table_save').html("* برای ذخیره ترتیب جدید باید روی دکمه روبرو کلیک نمایید:    <button class='btn btn-primary' onclick='saveSort();'>ذخیره ترتیب</button>");
}
});